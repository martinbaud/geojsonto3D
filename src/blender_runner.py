...
def verify_blender(blender_path: str | None) -> bool:
    ...
    try:
        result = subprocess.run([str(blender_path), "--version"], capture_output=True, text=True, timeout=10)
...
def main() -> int:
    ...
    parser.add_argument("--blender", "-b", type=str, help="Path to blender.exe (cached for future use)")
    ...
    parser.add_argument("--gui", "-g", action="store_true", help="Run with Blender GUI (default: background)")

    parser.add_argument("--configure", "-c", action="store_true", help="Reconfigure all settings")
    ...
    script_path = get_script_path(
        config, override=args.script if args.script != str(Path(__file__).parent / "run.py") else None
    )
    ...
    # Run Blender
    success = run_blender(blender_path, script_path, script_args, background=not args.gui)
